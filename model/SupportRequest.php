...
class SupportRequest {
    ...
    public function getCounselorUserId() {
    ...
    public function setAdminNote($note) {
        $this->admin_note = htmlspecialchars(trim($note), ENT_QUOTES, 'UTF-8');
    }
    ...
    private function update() {
        ...
        if ($this->admin_note === null || $this->admin_note === '') {
            $stmt->bindValue(':admin_note', null, PDO::PARAM_NULL);
        } else {
            $stmt->bindParam(':admin_note', $this->admin_note);
        }
    ...
    public function complete($note = null) {
        if (empty($this->id)) {
