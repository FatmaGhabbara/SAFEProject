<?php
// Détection automatique du chemin vers config.php
// Le fichier est dans: SAFEProject/SAFEProject/view/frontoffice/ajouter_signalement.php
// On doit trouver config.php qui peut être dans:
// - SAFEProject/SAFEProject/config.php (3 niveaux au-dessus)
// - SAFEProject/config.php (4 niveaux au-dessus depuis view/frontoffice)

$currentDir = __DIR__; // C:\xampp\htdocs\SAFEProject\SAFEProject\view\frontoffice
$configPath = null;

// Recherche robuste de config.php en remontant jusqu'à la racine (max 8 niveaux)
$checkDir = $currentDir;
for ($i = 0; $i < 8; $i++) {
...
if (!$configPath) {
	$msg = 'ajouter_signalement.php: config.php non trouvé. Recherché depuis: ' . $currentDir . '. Dernier candidat: ' . (isset($candidate) ? $candidate : '[néant]');
	error_log($msg);
...
if (!is_dir($modelPath)) {
    die('Erreur: Dossier model non trouvé. Vérifiez la structure du projet.');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if (!isset($_POST['type_id']) || trim($_POST['type_id']) === '') {
	...
	if (empty($errors)) {
		...
		if (!empty($result['errors']) && is_array($result['errors'])) {
			// Merge controller errors into the local $errors so they are displayed
		...
		if (isset($result['success']) && $result['success']) {
			...
			$message = $result['message'] ?? 'Erreur lors de la création du signalement';
			// Journaliser l'erreur serveur pour faciliter le débogage
			if (isset($result['message'])) {
				error_log('ajouter_signalement: create failed: ' . $result['message']);
			}
			if (isset($result['errors']) && is_array($result['errors'])) {
				foreach ($result['errors'] as $err) {
					error_log('ajouter_signalement: validation error: ' . $err);
				}
			}
		}
    } else {
						...
						<div class="form-card">
							...
							<form method="POST" id="signalement-form">
								...
								<div class="form-actions">
									...
									<a href="mes_signalements.php" class="button">← Retour à la liste</a>
								</div>
							</form>
						</div>
					</div>
